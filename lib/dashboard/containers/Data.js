...
export default class Data extends React.Component {
  ...
  addCollection(e, title, properties, user_id) {
  	e.preventDefault();
  	console.log(e, title, properties, user_id)
  	let collectionData = {};
  	...
  	collectionData.user_id = user_id;
		...
		xhr.open('POST', 'http://localhost:3000/api/collection');
	...
	deleteCollection(e, id) {
		...
		xhr.open('DELETE', 'http://localhost:3000/api/collection/' + collectionId);
		xhr.send();
		...
		xhrItem.open('DELETE', 'http://localhost:3000/api/collection_items/' + collectionId);
		xhrItem.send();
	}
